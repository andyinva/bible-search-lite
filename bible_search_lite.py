...
class BibleSearchProgram(QMainWindow):
    def __init__(self):
        ...
        self.user_data_service = UserDataService('database/user_data.db')
        ...
        self.load_config()
        self.load_groups()  # Load groups from database into dropdown
    ...
    def create_subject_controls(self):
        """DEPRECATED: Old subject controls - now handled by SubjectManager"""
        # This method is no longer used - kept for reference only
        """Create subject controls with Group and Subject dropdowns - SINGLE ROW LAYOUT"""
        controls_widget = QWidget()
        layout = QHBoxLayout(controls_widget)
        layout.setContentsMargins(0, 0, 0, 0)
        layout.setSpacing(5)

        # Acquire and Clear buttons first
        ...
        delete_verses_btn = QPushButton("Delete")
        delete_verses_btn.clicked.connect(self.delete_verses_from_subject)
        delete_verses_btn.setStyleSheet(self.get_button_style())
        ...
        self.subject_combo.addItems(["None", "Love", "Faith", "Hope", "Salvation"])
        new_subject_button = QPushButton("New Subject")
        new_subject_button.clicked.connect(self.on_new_subject)
        new_subject_button.setStyleSheet(self.get_button_style())

        delete_subject_button = QPushButton("Delete Subject")
        delete_subject_button.clicked.connect(self.on_delete_subject)
        delete_subject_button.setStyleSheet(self.get_button_style())

        # Find button
        find_button = QPushButton("Find")
        find_button.clicked.connect(self.on_find)
        find_button.setStyleSheet(self.get_button_style())

        # Add all controls left-to-right
        layout.addWidget(self.acquire_button)
        layout.addWidget(clear_button)
        layout.addWidget(delete_verses_btn)
        layout.addWidget(delete_verses_btn)
        layout.addWidget(group_label)
        layout.addWidget(self.group_combo)
        layout.addWidget(new_group_button)
        layout.addWidget(delete_group_button)
        layout.addWidget(subject_label)
        layout.addWidget(self.subject_combo)
        layout.addWidget(new_subject_button)
        layout.addWidget(delete_subject_button)
    ...
    def acquire_verses(self):
        """Acquire selected verses from ANY window (Search or Reading) to Subject"""
        if not self.current_subject_id:
            self.message_label.setText("⚠️ Please select a Subject first!")
            return
        
        # Collect verses from ALL windows (not just active)
        all_selected_verse_ids = []
        
        for window_id in ['search', 'reading']:
            if window_id in self.verse_lists:
                selected = self.verse_lists[window_id].get_selected_verses()
                all_selected_verse_ids.extend(selected)
        
        if not all_selected_verse_ids:
            self.message_label.setText("⚠️ No verses selected!")
            return
        
        print(f"Acquiring {len(all_selected_verse_ids)} verse(s) from all windows to subject ID {self.current_subject_id}")
        
        acquired_count = 0
        duplicate_count = 0
        
        # Process each selected verse
        for verse_id in all_selected_verse_ids:
            # Find the verse widget in either search or reading window
            verse_item = None
            source_window = None

            for window_id in ['search', 'reading']:
                if window_id in self.verse_lists:
                    if verse_id in self.verse_lists[window_id].verse_items:
                        # verse_items now returns (QListWidgetItem, VerseItemWidget) tuple
                        item, verse_item = self.verse_lists[window_id].verse_items[verse_id]
            ...
            # verse_item is already the VerseItemWidget (unpacked above on line 1144)
            # Extract verse info
            translation = verse_item.translation
            book = verse_item.book_abbrev
            chapter = verse_item.chapter
            verse_number = verse_item.verse_number
            ...
            # Save to database
            try:
                result = self.user_data_service.add_verse_to_subject(
                ...
                if result:
                    ...
                    # Add to Subject window (Window 4) UI
                    ...
                    duplicate_count += 1
                    print(f"  ⚠️ Duplicate: {translation} {verse_reference}")
            
            except Exception as e:
                print(f"  ❌ Error saving verse: {e}")
        
        # Clear selections from ALL windows after successful acquire
        for window_id in ['search', 'reading']:
            if window_id in self.verse_lists:
                self.verse_lists[window_id].select_none()
        
        # Update message
        if acquired_count > 0:
            if duplicate_count > 0:
                self.message_label.setText(f"✅ Saved {acquired_count} verse(s), {duplicate_count} duplicate(s) skipped")
            else:
                self.message_label.setText(f"✅ Saved {acquired_count} verse(s) to database")
        else:
            self.message_label.setText(f"⚠️ All verses were duplicates ({duplicate_count} skipped)")
        
        self.load_subject_verses()
        print(f"✅ Acquired {acquired_count} verses, {duplicate_count} duplicates")


    def on_group_changed(self, group_name):
        """Handle group selection - load subjects"""
        if not group_name or group_name == "Select Group...":
            self.current_group_id = None
            self.current_subject_id = None
        ...
        # Find group by name
        groups = self.user_data_service.get_all_groups()
        ...
        if group:
            ...
            subjects = self.user_data_service.get_subjects_by_group(self.current_group_id)
            
            # Update dropdown
            self.subject_combo.clear()
            self.subject_combo.addItem("Select Subject...")
            for subj in subjects:
                self.subject_combo.addItem(subj['subject_name'])
            
            print(f"Loaded {len(subjects)} subjects for '{group_name}'")
            self.message_label.setText(f"Loaded {len(subjects)} subject(s)")
            
            # Clear subject verses
            self.current_subject_id = None
            if 'subject' in self.verse_lists:
                        self.verse_lists['subject'].clear_verses()
    
    def on_subject_changed(self, subject_name):
        """Handle subject selection and load verses"""
        ...
        if hasattr(self, 'current_group_id') and self.current_group_id:
            subjects = self.user_data_service.get_subjects_by_group(self.current_group_id)
            subject = next((s for s in subjects if s['subject_name'] == subject_name), None)
            
            if subject:
                self.current_subject_id = subject['subject_id']
                print(f"Selected subject: {subject_name} (ID: {self.current_subject_id})")
            ...
            print("No group selected, cannot set subject")
            self.current_subject_id = None
            if 'subject' in self.verse_lists:
                        self.verse_lists['subject'].clear_verses()
    ...
    def clear_subject_display(self):
        """Clear Window 4 display but keep subject selection"""
        print(f"🧹 Clearing Window 4, subject_id={self.current_subject_id} stays selected")
        if "subject" in self.verse_lists:
            self.verse_lists["subject"].clear_verses()
        self.message_label.setText("Window 4 cleared - click any verse to refresh")


    def delete_verses_from_subject(self):
        """Delete selected verses from the current subject"""
        if not self.current_subject_id:
            self.message_label.setText("No subject selected")
            return
        
        if "subject" not in self.verse_lists:
            return
        
        selected_verse_ids = self.verse_lists["subject"].get_selected_verses()
        if not selected_verse_ids:
            self.message_label.setText("No verses selected to delete")
            return
        
        from PyQt6.QtWidgets import QMessageBox
        reply = QMessageBox.question(
            self, "Confirm Delete",
            f"Delete {len(selected_verse_ids)} verse(s) from this subject?",
        ...
        try:
            verses = self.user_data_service.get_verses_by_subject(self.current_subject_id)
            deleted_count = 0
            for verse_id in selected_verse_ids:
                try:
                    # Extract database ID from verse_id (e.g., "subject_db_28" -> 28)
                    db_id = int(verse_id.split("_")[-1])
                    self.user_data_service.delete_verse_from_subject(db_id)
                    deleted_count += 1
                except (ValueError, KeyError) as e:
                    print(f"Error deleting verse {verse_id}: {e}")
                    continue
            self.load_subject_verses()
            self.message_label.setText(f"Deleted {deleted_count} verse(s) from subject")
        except Exception as e:
            self.message_label.setText(f"Error deleting verses: {str(e)}")
            print(f"Delete error: {e}")

    def load_subject_verses(self):
        """Load verses for current subject from database"""
        if not self.current_subject_id:
            return
        
        # Clear existing verses in UI
        self.verse_lists['subject'].clear_verses()
        
        # Load from database
        verses = self.user_data_service.get_verses_by_subject(self.current_subject_id)
        
        print(f"Loading {len(verses)} verse(s) from database")
        
        # Add to UI
        for verse in verses:
            verse_id = f"subject_db_{verse['id']}"
        ...
        if len(verses) > 0:
            self.message_label.setText(f"Loaded {len(verses)} verse(s)")
            # Automatically activate Window 4 (Subject) after loading verses
            # This allows user to immediately use Ctrl+A or Copy without clicking
            self.set_active_window('subject')
            print("🎯 Auto-activated Window 4 (Subject Window)")
        else:
            self.message_label.setText("No verses in this subject")
...
"""
    ...
    def on_new_group(self):
        ...
        if ok and group_name.strip():
            try:
                result = self.user_data_service.create_group(group_name.strip())
                if result:
                    self.message_label.setText(f"Created group: {group_name}")
                    # Reload groups
                    self.load_groups()
                    # Select the new group
                    index = self.group_combo.findText(group_name.strip())
                    if index >= 0:
                        self.group_combo.setCurrentIndex(index)
                else:
                    self.message_label.setText("Group already exists")
            except Exception as e:
                self.message_label.setText(f"Error creating group: {e}")
    
    def on_new_subject(self):
        """Create a new subject in current group"""
        from PyQt6.QtWidgets import QInputDialog
        ...
        if ok and subject_name.strip():
            try:
                result = self.user_data_service.create_subject(
                ...
                if result:
                    ...
                    index = self.subject_combo.findText(subject_name.strip())
                    if index >= 0:
                        self.subject_combo.setCurrentIndex(index)
                else:
                    self.message_label.setText("Subject already exists in this group")
            except Exception as e:
                self.message_label.setText(f"Error creating subject: {e}")


    def on_delete_group(self):
        """Delete current group and all its subjects"""
        from PyQt6.QtWidgets import QMessageBox
        ...
        reply = QMessageBox.question(
            self,
            "Delete Group",
            f"Delete group '{group_name}' and all its subjects?\n\nThis cannot be undone!",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            QMessageBox.StandardButton.No
        )
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                result = self.user_data_service.delete_group(self.current_group_id)
                if result:
                    self.message_label.setText(f"Deleted group: {group_name}")
                    # Clear selections
                    self.current_group_id = None
                    self.current_subject_id = None
                    if 'subject' in self.verse_lists:
                    ...
                    self.message_label.setText("Failed to delete group")
            except Exception as e:
                self.message_label.setText(f"Error deleting group: {e}")
    
    def on_delete_subject(self):
        """Delete current subject and all its verses"""
        from PyQt6.QtWidgets import QMessageBox
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                result = self.user_data_service.delete_subject(self.current_subject_id)
                if result:
                    self.message_label.setText(f"Deleted subject: {subject_name}")
                    # Clear selection
                    self.current_subject_id = None
                    if 'subject' in self.verse_lists:
                        self.verse_lists['subject'].clear_verses()
                    ...
                    self.message_label.setText("Failed to delete subject")
            except Exception as e:
                self.message_label.setText(f"Error deleting subject: {e}")
    ...
    def load_groups(self):
        """Load groups from database into dropdown"""
        # Check if group_combo exists (old deprecated UI component)
        if not hasattr(self, 'group_combo'):
            # Group dropdown no longer exists - using new SubjectManager
            return
        ...
        try:
            ...
            for group in groups:
                self.group_combo.addItem(group['group_name'])
